...
echo("Required Minimum Frame Gap:",ceil( (nema_width[motors[x_axe]]*2+bearing_block_width[z_bearing]+2)/10 )*10);
echo("Required Minimum Frame Bolt Lenght:",ceil( (nema_width[motors[x_axe]]*2+bearing_block_width[z_bearing]+frame_thickness*2+frame_nut_thickness+2)/10 )*10);
if (ceil( (nema_width[motors[x_axe]]*2+bearing_block_width[z_bearing]+2)/10 )*10 > frame_gap){
echo ("============================================");
...
module x_axis_leadscrew(show){
	if(show=="motor"){
		//motor mount
		translate([nema_width[motors[x_axe]]/2,-bearing_block_width[z_bearing]/2,x_rod_mount_thickness/2-motor_holder_thickness/2]) rotate([0,0,180]) motor_mount(motors[x_axe],1);
	}	
	if(show=="idler"){

	}
		//x_rod 
		translate([nema_width[motors[x_axe]]/2-x_rod_distance,-nema_width[motors[x_axe]]/2-bearing_block_width[z_bearing]/2-motor_edge_gap,0]){
			difference(){
				cube([x_rod_mount_lenght,x_rod_mount_width,x_rod_mount_thickness], center=true);
		...
		difference(){
			//z_block
			translate([0,0,40]) cube([x_block_z_rod_thickness,bearing_block_width[z_bearing],80],center=true);
